...
export const calculatePrice = async (req, res) => {
  ...
  try {
    ...
    // Get zones for both pincodes first
    const fromZone = zoneForPincode(String(fromPincode));
    const toZone = zoneForPincode(String(toPincode));
    
    if (!fromZone || !toZone) {
      console.log(`[${rid}] Invalid zones: fromZone=${fromZone}, toZone=${toZone}`);
      return res.status(400).json({
        success: false,
      ...
      .aggregate([
        {
          $match: {
            ...
            servicableZones: {
              $all: [fromZone.toUpperCase(), toZone.toUpperCase()]
            }
          },
        },
        {
    ...
    const temporaryTransporterRaw = await Promise.all(
      temporaryTransporters.map(async (tempTransporter) => {
        ...
        let unitPrice =
          priceChart?.[originZone]?.[destZone] ?? priceChart?.[destZone]?.[originZone];
